        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-4">
            ...
            <div class="mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('Archived Requests') }}</h1>
                        <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('View your cancelled and rejected sourcing requests.') }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-base font-bold text-slate-900 dark:text-white">{{ __('Archived List') }}</h3>
                                    <p class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                                        <span class="font-semibold text-red-500">{{ $sourcingRequests->total() }}</span> {{ Str::plural(__('request'), $sourcingRequests->total()) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- Filters --}}
                        <form action="{{ route('client.sourcing-requests.archived') }}" method="GET" id="filterForm" class="w-full lg:w-auto">
                ...
                <div class="p-6">
                    @if ($sourcingRequests->isEmpty())
                        ...
                        <div class="text-center py-20 px-6">
                            <div class="mx-auto w-20 h-20 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center mb-6">
                                <svg class="w-10 h-10 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                </svg>
                            </div>
                            <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">{{ __('No archived requests') }}</h3>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mb-6 max-w-sm mx-auto">{{ __('You have no cancelled or rejected requests.') }}</p>
                                    ...
                                    @foreach ($sourcingRequests as $request)
                                        <tr class="hover:bg-[#EF7722]/5 dark:hover:bg-[#EF7722]/10 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @php
                                                    $statusConfig = [
                                                        'cancelled' => ['color' => '#EF4444', 'icon' => 'M6 18L18 6M6 6l12 12'], // Red
                                                        'rejected' => ['color' => '#EF4444', 'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z'], // Red alert
                                                    ];
                                                    $statusData = $statusConfig[$request->status] ?? ['color' => '#64748B', 'icon' => 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'];
                                                @endphp
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <a href="{{ route('client.sourcing-requests.show', $request) }}" 
                                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#EBEBEB] hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-lg transition-colors duration-200 shadow-sm">
                                                        {{ __('View Details') }}
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                                        </svg>
                                                    </a>
                                                    {{-- Allow Deletion for cleanup if needed --}}
                                                    <form action="{{ route('client.sourcing-requests.destroy', $request) }}" method="POST" onsubmit="return confirm('{{ __('Delete permanently?') }}');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" 
                                                                class="inline-flex items-center justify-center p-1.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors duration-200"
                                                                title="{{ __('Delete') }}">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                                            </svg>
