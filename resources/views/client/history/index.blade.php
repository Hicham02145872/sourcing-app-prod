<x-app-layout :breadcrumb="[
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-900">{{ __('Activity History') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ __('A chronological overview of all your activities and transactions') }}</p>
            </div>
            <div class="flex items-center gap-3">
                <button type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/>
                    </svg>
                    {{ __('Filter') }}
                </button>
                <button type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 border border-transparent rounded-lg shadow-sm transition-all">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($timeline->isEmpty())
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <div class="text-center py-20 px-6">
                        <div class="w-24 h-24 bg-gradient-to-br from-violet-100 to-purple-100 rounded-full flex items-center justify-center mx-auto mb-6">
                            <svg class="w-12 h-12 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                        </div>
                        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('No Activity Yet') }}</h3>
                        <p class="text-sm text-gray-600 max-w-md mx-auto mb-6">{{ __('Your activity timeline is empty. Start by creating requests and orders to see your history here.') }}</p>
                        <a href="{{ route('client.dashboard') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white text-sm font-semibold rounded-lg shadow-md transition-all">
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 bg-gradient-to-r from-violet-50 to-purple-50 border-b border-gray-200">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-base font-semibold text-gray-900">{{ __('Activity Timeline') }}</h3>
                            <span class="ml-auto px-2.5 py-0.5 text-xs font-semibold text-violet-700 bg-violet-100 rounded-full">
                            ...
                            <div class="space-y-6">
                                @foreach($timeline as $index => $event)
                                    <div class="relative flex items-start gap-4 group">
                                        ...
                                        {{-- Event card --}}
                                        <div class="flex-1 p-4 bg-gradient-to-r from-gray-50 to-gray-50 hover:from-violet-50 hover:to-purple-50 border border-gray-200 hover:border-violet-300 rounded-xl transition-all duration-200 group-hover:shadow-md">
                                            <div class="flex items-start justify-between mb-2">
                                                <div>
                                                    <h4 class="text-sm font-bold text-gray-900 mb-1">{{ $event['title'] }}</h4>
                                                    <p class="text-xs text-gray-600">{{ $event['date']->format('F j, Y') }}</p>
                                                </div>
                                                <span class="px-2.5 py-1 text-xs font-semibold text-violet-700 bg-violet-100 rounded-full">
                                            ...
                                            @if(isset($event['metadata']))
                                                <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-200">
                                                    <span class="text-xs text-gray-500">{{ __('Details') }}:</span>
                                                    @foreach($event['metadata'] as $key => $value)
                                                        <span class="px-2 py-0.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded">
                                                            {{ $key }}: {{ $value }}
                                                        </span>
                                                    @endforeach
                                                </div>
