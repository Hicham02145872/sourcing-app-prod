        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('notificationCenter', () => ({
                ...
                showToast(notification) {
                    // Implémenter un système de toast si nécessaire
                },
...
<body class="font-sans antialiased bg-gray-50" x-data="{ sidebarOpen: false }" x-cloak>
    ...
    <div class="lg:pl-64 min-h-screen flex flex-col">
        <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-gray-200/80 shadow-sm">
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    ...
                    <div class="hidden sm:flex sm:items-center sm:gap-4" x-data="notificationCenter">
                        <div class="relative">
                            <button @click="open = !open; if(open) fetchNotifications()" 
                                    class="relative p-2.5 rounded-xl hover:bg-gray-100 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 group"
                                    :class="{ 'bg-gray-100': open }">
                                <svg class="h-6 w-6 text-gray-700 group-hover:text-gray-900 transition-colors" 
                            ...
                            <div x-show="open" 
                                 ...
                                 class="absolute right-0 mt-3 w-[420px] bg-white rounded-2xl shadow-2xl border border-gray-200/80 overflow-hidden z-50"
                                 style="display: none;">
                                
                                {{-- En-tête Amélioré --}}
                                <div class="bg-gradient-to-r from-indigo-50 to-purple-50 px-6 py-4 border-b border-gray-200/80">
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <button @click="markAllAsRead()" 
                                                    ...
                                                    class="text-xs font-medium text-indigo-600 hover:text-indigo-700 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-all"
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex items-center gap-2">
                                            <button @click="filter = 'all'" 
                                                    :class="filter === 'all' ? 'bg-white text-indigo-600 shadow-sm' : 'bg-white/50 text-gray-600 hover:bg-white'"
                                                ...
                                                <span class="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold"
                                                      :class="filter === 'all' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-200 text-gray-600'"
                                                      x-text="notifications.length"></span>
                                            </button>
                                            <button @click="filter = 'unread'" 
                                                    :class="filter === 'unread' ? 'bg-white text-indigo-600 shadow-sm' : 'bg-white/50 text-gray-600 hover:bg-white'"
                                                    class="flex-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all">
                                                Non lues
                                                <span class="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold"
                                                      :class="filter === 'unread' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-200 text-gray-600'"
                                                      x-text="unreadCount"></span>
                                            </button>
                                            <button @click="filter = 'read'" 
                                                    :class="filter === 'read' ? 'bg-white text-indigo-600 shadow-sm' : 'bg-white/50 text-gray-600 hover:bg-white'"
                                                    class="flex-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all">
                                                Lues
                                            </button>
                                        </div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="text" 
                                                   ...
                                                   class="w-full pl-9 pr-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                        </div>
                                    </div>
                                </div>
                                ...
                                {{-- Message d'Erreur --}}
                                <div x-show="error" class="mx-4 my-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                                ...
                                <div class="max-h-[480px] overflow-y-auto scrollbar-thin" 
                                    ...
                                    <template x-for="(n, index) in filteredNotifications" :key="n.id || index">
                                        <div class="notification-item relative group border-b border-gray-100 last:border-0 hover:bg-gradient-to-r hover:from-gray-50 hover:to-indigo-50/30"
                                               ...
                                               class="flex items-start gap-3 px-6 py-4">
                                                ...
                                                <div class="flex-shrink-0 mt-0.5 relative">
                                                    ...
                                                    <span x-show="!n.read_at" 
                                                          class="absolute -top-1 -right-1 w-3 h-3 bg-indigo-600 rounded-full ring-2 ring-white animate-pulse"></span>
                                                </div>
                                                ...
                                                <button @click="deleteNotification(n.id, $event)"
                                                        class="flex-shrink-0 opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-100 transition-all duration-200"
                                                        title="Supprimer">
                                ...
                                <div class="border-t border-gray-200 bg-gray-50 px-6 py-3 flex items-center justify-between"
                                    ...
                                    <button @click="clearAll()"
                                            class="inline-flex items-center gap-1.5 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-all">
                                    ...
                                    <a href="{{ route('notifications.index') }}" 
                                       class="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-all">
                                        Voir tout
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center gap-3 ml-4">
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" 
                                    class="flex items-center gap-2.5 p-2 pr-3 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1">
                                <div class="w-9 h-9 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-xl flex items-center justify-center text-white font-bold shadow-lg ring-2 ring-white">
                                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                                </div>
                                <div class="hidden sm:block text-left">
                                    <p class="text-sm font-semibold text-gray-900 leading-none">{{ auth()->user()->name }}</p>
                                    <p class="text-xs text-gray-500 mt-0.5">{{ ucfirst(auth()->user()->role ?? 'Utilisateur') }}</p>
                                </div>
                            ...
                            <div x-show="open" 
                                ...
                                <div class="px-4 py-3 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-gray-200">
                                    ...
                                    <span class="inline-flex items-center mt-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">
                                ...
                                <div class="border-t border-gray-200">
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" 
                                                class="flex items-center gap-3 w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors font-medium">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex items-center gap-6">
                        <a href="#" class="text-sm text-gray-600 hover:text-indigo-600 transition-colors font-medium">Confidentialité</a>
                        <a href="#" class="text-sm text-gray-600 hover:text-indigo-600 transition-colors font-medium">Conditions</a>
                        <a href="#" class="text-sm text-gray-600 hover:text-indigo-600 transition-colors font-medium">Support</a>
                    </div>
                </div>
            </div>
        </footer>
