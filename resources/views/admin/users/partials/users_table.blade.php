<tbody class="bg-white divide-y divide-slate-200" id="users-table-body" x-data="{ expandedId: null }">
    @forelse ($users as $user)
        <tr class="hover:bg-slate-50 transition-colors group cursor-pointer" 
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end gap-2">
                    <button type="button" 
                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-bold rounded-md transition-all duration-200"
                            :class="expandedId === {{ $user->id }} ? 'bg-orange-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-orange-50 hover:text-orange-600'">
        ...
        <tr x-cloak x-show="expandedId === {{ $user->id }}" class="bg-slate-50/80 transition-all duration-300">
            <td colspan="5" class="px-0 py-0 border-0">
                <div class="p-6 border-b border-slate-200 shadow-inner">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                            <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                                <h4 class="text-[11px] font-extrabold uppercase tracking-widest text-slate-600 flex items-center gap-2">
                                ...
                                <span class="bg-white px-2 py-0.5 rounded text-[10px] font-bold text-slate-400 border border-slate-200">
                                    {{ $user->sourcingRequests->count() }}
                                </span>
                            </div>
                            <div class="flex-1 max-h-[250px] overflow-y-auto">
                                @forelse($user->sourcingRequests as $request)
                                    <div class="px-4 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors flex items-center justify-between group/item">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-bold text-slate-900 truncate max-w-[200px]">{{ $request->product_name }}</span>
                                            <span class="text-[10px] text-slate-400">{{ $request->created_at->format('d M Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            <span class="text-[10px] font-bold px-2 py-0.5 rounded text-slate-500 bg-slate-100 border border-slate-200 uppercase truncate max-w-[100px]">
                                                {{ $request->status }}
                                            </span>
                                            <a href="{{ route('admin.sourcing-requests.show', $request) }}" class="text-orange-600 hover:text-orange-700 p-1.5 rounded hover:bg-orange-50 lg:opacity-0 group-hover/item:opacity-100 transition-all shadow-sm bg-white border border-slate-100">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                            </a>
                                        </div>
                                    </div>
                                @empty
                                    <div class="px-4 py-8 text-center bg-slate-50/30">
                                    ...
                                    <div class="px-4 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors flex items-center justify-between group/item">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-bold text-slate-900 flex items-center gap-2">
                                                #{{ $order->id }} 
                                                <span class="text-[11px] text-blue-600">{{ number_format($order->total_amount, 2) }} MAD</span>
                                            </span>
                                            <span class="text-[10px] text-slate-400">{{ $order->created_at->format('d M Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            <span class="text-[10px] font-bold px-2 py-0.5 rounded text-blue-600 bg-blue-50 border border-blue-100 uppercase truncate max-w-[100px]">
                                                {{ str_replace('_', ' ', $order->status) }}
                                            </span>
                                            <a href="{{ route('admin.sourcing-orders.show', $order) }}" class="text-blue-600 hover:text-blue-700 p-1.5 rounded hover:bg-blue-50 lg:opacity-0 group-hover/item:opacity-100 transition-all shadow-sm bg-white border border-slate-100">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                            </a>
                                        </div>
                                    </div>
    ...
    @empty
        <tr>
            <td colspan="5" class="px-6 py-12 text-center bg-slate-50/20">
                <div class="flex flex-col items-center justify-center">
                    <div class="h-12 w-12 rounded-full bg-white border border-slate-100 flex items-center justify-center mb-3 shadow-sm">
                        <svg class="w-6 h-6 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    </div>
                    <h3 class="text-sm font-bold text-slate-900">{{ __('No clients found') }}</h3>
                    <p class="text-xs text-slate-500 mt-1">{{ __('Try adjusting your search or filters.') }}</p>
                </div>
            </td>
        </tr>
    @endforelse
