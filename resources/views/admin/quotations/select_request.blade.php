            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('Sourcing Requests') }}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">{{ __('In Review') }}</p>
                            <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2">{{ $sourcingRequestsInReview }}</p>
                        ...
                        <div class="w-12 h-12 bg-[#EF7722]/10 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6 text-[#EF7722]" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                            </svg>
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">{{ __('Pending Quotations') }}</p>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide">{{ __('This Month') }}</p>
                            <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2">{{ $sourcingRequestsThisMonth }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ __('Active Requests') }}</h3>
                                    <p class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                                        <span class="font-semibold text-[#EF7722]">{{ count($sourcingRequests) }}</span> {{ Str::plural('request', count($sourcingRequests)) }} {{ __('ready for quotation') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                @if ($sourcingRequests->isEmpty())
                    ...
                    <div class="text-center py-20 px-6">
                        ...
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">{{ __('No Sourcing Requests') }}</h3>
                                    ...
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-xs text-slate-600 dark:text-slate-400">{{ $request->created_at->format('M d, Y') }}</div>
                                                </div>
                                            </div>
                                        </td>
                                            ...
                                            <div class="max-w-xs">
                                                ...
                                                @if($request->product_url)
                                                    <a href="{{ $request->product_url }}" target="_blank" 
                                                       class="inline-flex items-center gap-1 text-xs text-[#EF7722] hover:text-[#FAA533] mt-1 transition-colors font-semibold">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                @endif
                                                @if($request->note)
                                                    <p class="text-xs text-slate-600 dark:text-slate-400 mt-1 line-clamp-1">
                                                        {{ $request->note }}
                                                    </p>
                                                @endif
                                            </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <div class="text-sm font-semibold text-slate-900 dark:text-white">{{ $request->user->name }}</div>
                                                    <div class="text-xs text-slate-600 dark:text-slate-400">{{ $request->user->email }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <a href="{{ route('admin.quotations.create', $request) }}" 
                                               class="inline-flex items-center gap-1.5 px-4 py-2.5 bg-gradient-to-r from-[#EF7722] to-[#FAA533] hover:from-[#FAA533] hover:to-[#EF7722] text-white text-xs font-bold rounded-lg transition-all shadow-md hover:shadow-lg group">
